...
interface LeaderboardEntry {
  playerName: string;
  totalGames: number;
  averageThrows: number;
  bestThrows: number;
  lastPlayed: string;
}

const SALStats: React.FC = () => {
  ...
  return (
    <div className="sal-stats">
      <div className="sal-stats-nav">
        <button onClick={() => navigate('/')} className="sal-stats-back-btn">
          ← Back to Games
        </button>
        <button onClick={() => navigate('/games/sal')} className="sal-stats-play-btn">
          Play Game
        </button>
      </div>

      <div className="sal-stats-header">
        <h1>Snake and Ladder Statistics</h1>
      ...
      {error && (
        <div className="sal-stats-error">
          <p>{error}</p>
        </div>
      )}

      {loading ? (
        <div className="sal-stats-loading">
        ...
        <>
          {activeTab === 'history' && (
            <div className="sal-stats-content">
                    ...
                    <tbody>
                      {leaderboard.map((entry, index) => (
                        ...
                        >
                          ...
                          <td>{entry.averageThrows.toFixed(1)}</td>
                          <td className="best-throws">{entry.bestThrows}</td>
