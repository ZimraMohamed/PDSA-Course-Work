...
// 2. Add Entity Framework (SQLite)
builder.Services.AddDbContext<PDSADbContext>(options =>
    options.UseSqlite(builder.Configuration.GetConnectionString("DefaultConnection") 
        ?? "Data Source=pdsa.db"));

// 3. Add Scoped Services for Algorithms
builder.Services.AddScoped<TSPGameService>();
builder.Services.AddScoped<TrafficGameService>();

// 4. FIXED CORS: Allow everything so Nginx can proxy requests without security blocks
builder.Services.AddCors(options =>
{
...
using (var scope = app.Services.CreateScope())
{
    ...
    command.CommandText = "SELECT name FROM sqlite_master WHERE type='table' ORDER BY name;";
    var reader = command.ExecuteReader();
    Console.WriteLine("Current Database Tables:");
    while (reader.Read())
    {
        Console.WriteLine($"  - {reader.GetString(0)}");
    }
...
// CRITICAL FIX: Disabled HTTPS Redirection because EC2 is running on HTTP (Port 80)
// app.UseHttpsRedirection(); 
...
static void SeedEightQueensSolutions(PDSADbContext context)
{
    ...
    // Run the Threaded algorithm
    var (threadedSolutions, threadedTime) = EQPSolver.SolveThreaded(8);

    var roundNumber = 1; 
    
    // Save Sequential Time
    context.EQPAlgoTimes.Add(new EQPAlgoTime
    {
        DateExecuted = DateTime.UtcNow.ToString("o"),
        AlgorithmType = "Sequential Backtracking",
        TimeTaken_ms = sequentialTime,
    ...
    foreach (var solution in sequentialSolutions)
    {
        ...
        context.EQPSolutions.Add(new EQPSolution
        {
            Solution_Text = string.Join(",", positions),
            IsFound = false,
            PlayerID = null,
            DateFound = null
        });
    ...
    context.SaveChanges();
    Console.WriteLine($"Successfully seeded {sequentialSolutions.Count} solutions.");
}